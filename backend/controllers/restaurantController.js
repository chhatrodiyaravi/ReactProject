...
const normalizeRestaurantPayload = (body = {}) => {
  ...
  if (!payload.restaurantName && payload.name) {
    payload.restaurantName = payload.name;
  }

  if (payload.address && typeof payload.address === "object") {
    payload.city = payload.city || payload.address.city || "";
    payload.state = payload.state || payload.address.state || "";
    payload.pincode = payload.pincode || payload.address.zipCode || "";
  }

  if (!payload.categories && Array.isArray(payload.cuisine)) {
    payload.categories = payload.cuisine;
  }
...
export const createRestaurant = async (req, res) => {
  try {
    ...
    if (req.file) {
      payload.image = await resolveImageUrl(req.file, "restaurants");
    }
